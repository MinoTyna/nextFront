  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log("📥 Données reçues :", data); // <— à vérifier dans la console

        // On vérifie que c’est bien un tableau avant de mettre à jour l’état
        if (Array.isArray(data)) {
          setNotifications(data);
        } else if (Array.isArray(data.notifications)) {
          setNotifications(data.notifications);
        } else {
          console.warn("⚠️ Format inattendu des notifications :", data);
