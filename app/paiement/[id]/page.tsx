...
export default function DetailClientPage() {
  ...
  const handlePaiement = async () => {
    ...
    const montantNumber = parseFloat(montant); // plus sûr que Number()
    if (!montantNumber || montantNumber < 100000)
      return toast.error("Montant invalide (min 100 000 Ar).");

    const payload = {
      client: clientId,
      Paiement_montant: montantNumber.toFixed(0), // converti en string entier pour Django
      ...
      {showPaiementModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="md:w-1/2 w-full max-w-lg bg-white border p-6 rounded-xl shadow-lg relative">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className={`w-full px-3 py-2 rounded border ${
                  montant && parseInt(montant) < 100000
                    ? "border-red-500 bg-red-100" // 🔴 rouge si <100000
            ...
            {client && (
              <button
                disabled={isPaying || !montant || parseInt(montant) < 100000} // 🔹 désactivé si montant invalide
                className={`mt-4 px-4 py-2 text-white rounded w-full ${
                  isPaying || !montant || parseInt(montant) < 100000
